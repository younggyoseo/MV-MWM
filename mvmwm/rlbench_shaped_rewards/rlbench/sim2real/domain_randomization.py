...
class RandomizationConfig(object):
    ...
    def should_randomize(self, obj_name: str):
        return (
            (self.whitelist is None and len(self.blacklist) == 0)
            or (self.whitelist is not None and obj_name in self.whitelist)
            or (obj_name not in self.blacklist)
        ) and (self.randomize_arm or "panda" not in obj_name.lower())
...
class VisualRandomizationConfig(RandomizationConfig):
    def __init__(
        ...
        if not os.path.exists(image_directory):
            raise NotADirectoryError(
                "The supplied image directory (%s) does not exist!" % image_directory
            )
        self._imgs = np.array(
            [glob.glob(os.path.join(image_directory, e)) for e in EXTENSIONS]
        )
    ...
    def select(self, samples: int) -> np.ndarray:
        return self._imgs[:samples]
