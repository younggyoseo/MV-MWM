...
flags.DEFINE_string(
    "save_dir", "/tmp/rlbench_videos/", "Where to save the generated videos."
)
flags.DEFINE_list("tasks", [], "The tasks to record. If empty, all tasks are recorded.")
...
flags.DEFINE_string(
    "textures_path", "", "Where to locate textures if using domain randomization."
)
flags.DEFINE_boolean("headless", True, "Run in headless mode.")
flags.DEFINE_list("camera_resolution", [1280, 720], "The camera resolution")
...
def main(argv):
    ...
    task_classes = [task_file_to_task_class(task_file) for task_file in task_names]
